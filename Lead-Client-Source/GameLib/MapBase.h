...
class CMapBase : public CScreen
{
	...
	protected:
		...
		virtual void	OnSetEnvironmentDataPtr() = 0;	// Called when Environment data is set.
	...
	public:
		...
		bool IsCopiedMap() const { return m_strParentMapName.length() > 0; }		// Is it a copy map with "original data separate"?
		const std::string& GetParentMapName() const { return m_strParentMapName; }	// Returns the name of the map containing the original data (must be set as required for copy maps)
		const std::string& GetMapDataDirectory() const { return IsCopiedMap() ? m_strParentMapName : m_strName; }

	protected:
		EMAPTYPE				m_eType;				// Map types... Currently, there are Indoor and Ourdoor.
		std::string				m_strName;				// Map name. Can a map name be a map identifier?
		std::string				m_strParentMapName;		// Name of the original map. If this value is set, all map data is actually read from the ParentMap/* path. I passed the function that reads only part of the map because I don't need it.
		bool					m_bReady;
