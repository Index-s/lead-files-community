    ...
    def declare_builtin(self, name, pos):
        if not hasattr(builtins, name) \
            ...
            if self.has_import_star:
                ...
                # unknown - assume it's not a builtin but a dynamic set variable.
                # todo : if it is not a variable but a method or function etc, how can i handle it?
                entry = self.declare_var(name, py_object_type, pos)
                print py_object_type
                return entry
        if Options.cache_builtins:
