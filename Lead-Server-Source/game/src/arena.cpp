...
void CArenaMap::SendArenaMapListTo(LPCHARACTER pChar, DWORD mapIdx)
{
	...
	for (; iter != m_listArena.end(); iter++)
	{
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("MapInfo"), mapIdx,
	...
	else if (pCharA != NULL && pCharB != NULL)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			else
			{
				...
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s %d : %d %s"), pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s has won."), pCharA->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("%s %d : %d %s"), pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
