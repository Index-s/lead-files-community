...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		...
		{
			pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d %s %s"), m_vec_quiz[i][j].level, m_vec_quiz[i][j].Quiz, m_vec_quiz[i][j].answer ? LC_TEXT("TRUE") : LC_TEXT("FALSE"));
		}
	}
...
bool COXEventManager::GiveItemToAttender(DWORD dwItemVnum, ItemStackType count)
{
	itertype(m_map_attender) iter = m_map_attender.begin();
