...
//		BindDesc(NULL);
	...
	if (party)
	{
		...
		SetParty(NULL); // You don’t have to, but it’s safe .
	}
...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Equipped items cannot be sold in personal stores."));
				return;
			}

			if (true == pkItem->isLocked())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Items in use cannot be sold in personal stores."));
				return;
			}
...
void CHARACTER::ComputePoints()
{
	...
	// ComputePoints Initializes all attribute values ​​of the character and ,
	// item , Because all attribute values ​​related to buffs, etc. are recalculated. ,
	// Dragon soul stone system ActiveDeck The attribute values ​​of all dragon soul stones must be reapplied. .
		...
		case POINT_EXP:
			{
				...
				else
				{
					...
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have gained %d exp."), amount);
		...
		case POINT_STAMINA:
			{
				...
				if (val == 0)
				{
					// Stamina There is no place so let’s walk !
					SetNowWalking(true);
				}
				else if (prev_val == 0)
				{
...
void CHARACTER::fishing()
{
	...
	// Try fishing in a property you can't catch. ?
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Not a place for fishing"));
			return;
		}
	}
...
bool CHARACTER::WarpToPID(DWORD dwPID)
{
	...
	else
	{
		...
		else
		{
			...
			if (test_server) 
				ChatPacket(CHAT_TYPE_PARTY, LC_TEXT("sent find position packet for teleport"));
		}
	}
	return true;
}
...
bool CHARACTER::IsHack(bool bSendMsg, bool bCheckShopOwner, int limittime)
{
	...
	if (iPulse - GetSafeboxLoadTime() < PASSES_PER_SEC(limittime))
	{
		...
		if (test_server)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[TestOnly]Pulse %d LoadTime %d PASS %d"), iPulse, GetSafeboxLoadTime(), PASSES_PER_SEC(limittime));
		return true; 
	}
